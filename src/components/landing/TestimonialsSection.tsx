...
export default function TestimonialsSection() {
  ...
  return (
    <section className="bg-az-black py-20 md:py-28">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: EASE }} className="font-mono text-xs text-cyan uppercase tracking-[0.1em] mb-4">Testimonials</motion.p>
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1, duration: 0.6, ease: EASE }} className="font-playfair text-3xl sm:text-4xl font-bold text-white">What Our Clients Say</motion.h2>
        </div>

        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: EASE }} className="relative" onMouseEnter={() => setIsAutoPlaying(false)} onMouseLeave={() => setIsAutoPlaying(true)}>
          <button onClick={prev} className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 lg:-translate-x-6 w-12 h-12 rounded-full bg-[#141414] border border-[#2A2A2A] items-center justify-center text-gray-400 hover:text-white hover:bg-[#1A1A1A] transition-colors z-10" aria-label="Previous"><ChevronLeft size={20} /></button>
          ...
          <div className="overflow-hidden">
            <AnimatePresence mode="wait">
              <motion.div key={current} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, ease: EASE }} className="bg-[#141414] border border-[#2A2A2A] rounded-2xl p-8 sm:p-10 min-h-[280px] flex flex-col justify-between">
                <div>
                  <Quote size={48} className="text-cyan opacity-10 mb-4" />
                  <p className="text-gray-200 text-base sm:text-lg italic leading-relaxed">&ldquo;{t.quote}&rdquo;</p>
                </div>
          ...
          <div className="flex items-center justify-center gap-2 mt-6">
            {TESTIMONIALS.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)} className={`w-2 h-2 rounded-full transition-colors duration-200 ${i === current ? 'bg-cyan' : 'bg-gray-700 hover:bg-gray-500'}`} aria-label={`Go to testimonial ${i + 1}`} />
            ))}
          </div>
        </motion.div>
      </div>
