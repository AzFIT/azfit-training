    ...
    <section className="bg-[#F8F9FA] py-20 lg:py-24">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1, duration: 0.5, ease: EASE }} className="font-playfair text-3xl sm:text-4xl font-bold text-[#111827]">What Our Clients Say</motion.h2>
        </div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, ease: EASE }} className="relative" onMouseEnter={() => setIsAutoPlaying(false)} onMouseLeave={() => setIsAutoPlaying(true)}>
          ...
          <div className="overflow-hidden">
            <AnimatePresence mode="wait">
              <motion.div key={current} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4, ease: EASE }} className="bg-white border border-[#E5E7EB] rounded-2xl p-8 sm:p-10 min-h-[280px] flex flex-col justify-between">
                <div>
                  <Quote size={48} className="text-cyan opacity-10 mb-4" />
                  <p className="text-[#111827] text-base sm:text-lg italic leading-relaxed">&ldquo;{t.quote}&rdquo;</p>
                </div>
          ...
          <div className="flex items-center justify-center gap-2 mt-6">
            {TESTIMONIALS.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)} className={`w-2 h-2 rounded-full transition-colors duration-200 ${i === current ? 'bg-cyan' : 'bg-[#E5E7EB] hover:bg-[#D1D5DB]'}`} aria-label={`Go to testimonial ${i + 1}`} />
            ))}
          </div>
        </motion.div>
      </div>
