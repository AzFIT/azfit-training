...
export default function HowItWorksSection() {
  return (
    <section id="how-it-works" className="bg-[off-white-2] py-20 lg:py-24">
                ...
                <div className="hidden md:grid md:grid-cols-2 md:gap-8 md:w-full md:items-start">
                  <div className={`${isLeft ? 'text-right pr-8' : 'col-start-2 pl-8'}`}>
                    <div className={`inline-flex flex-col ${isLeft ? 'items-end' : 'items-start'}`}>
                      <div className="bg-white rounded-xl p-6 shadow-[0_4px_16px_rgba(0,0,0,0.06)] border border-[gray-200] max-w-[380px]">
                        <h3 className="font-semibold text-lg text-[gray-950] mb-2">{step.title}</h3>
                        <p className="text-[gray-550] text-sm leading-relaxed">{step.description}</p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="md:hidden ml-12 mr-0">
                  <div className="bg-white rounded-xl p-5 shadow-[0_4px_16px_rgba(0,0,0,0.06)] border border-[gray-200]">
                    <h3 className="font-semibold text-base text-[gray-950] mb-1.5">{step.title}</h3>
                    <p className="text-[gray-550] text-sm leading-relaxed">{step.description}</p>
                  </div>
                </div>
