  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => { setCompareMode(!compareMode); setSelectedForCompare([]); }}
            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all ${compareMode ? 'bg-cyan text-white' : 'bg-[#1A1A1A] text-dark-secondary hover:text-dark-primary'}`}>
      ...
      {compareMode && selectedForCompare.length === 2 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#141414] border border-dark-border rounded-xl p-5">
          ...
          {filtered.map((photo, i) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative bg-[#141414] border rounded-xl overflow-hidden cursor-pointer transition-all ${isSelected ? 'border-cyan ring-2 ring-[#00AEEF]/30' : 'border-dark-border hover:border-dark-subtle'}`}
                onClick={() => {
      ...
      {viewingPhoto && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }}
            className="bg-[#141414] border border-dark-border rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between p-4 border-b border-dark-border">
                ...
                <button onClick={() => setViewingPhoto(null)} className="w-8 h-8 rounded-lg bg-[#1A1A1A] hover:bg-dark-hover flex items-center justify-center text-dark-secondary transition-colors">
                  <X size={16} />
                </button>
              </div>
              <div className="p-4">
