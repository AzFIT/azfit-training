...
export function NotesTab() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 h-[calc(100vh-280px)] min-h-[500px]">
      {/* Left — Note List */}
      <div className="bg-[#141414] border border-dark-border rounded-xl flex flex-col overflow-hidden">
        <div className="p-4 border-b border-dark-border space-y-3">
          <div className="flex items-center gap-2">
            <div className="flex-1 flex items-center bg-[#1A1A1A] rounded-lg border border-dark-border px-3">
              <Search size={14} className="text-dark-muted flex-shrink-0" />
              <input
                type="text" placeholder="Search notes..." value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="flex-1 bg-transparent text-sm text-dark-primary placeholder-[#6B6B6B] py-2 px-2 outline-none"
              />
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {filtered.map((note) => (
            <button
              ...
              className={`w-full text-left p-3 rounded-lg transition-all ${selected?.id === note.id && !isAdding ? 'bg-[rgba(0,174,239,0.15)] border border-[rgba(0,174,239,0.3)]' : 'bg-[#1A1A1A] border border-dark-divider hover:bg-dark-hover'}`}
            >
      ...
      <div className="lg:col-span-2 bg-[#141414] border border-dark-border rounded-xl p-5 overflow-y-auto">
        {isAdding ? (
          <div className="space-y-4">
            ...
            <div>
              ...
              <select value={newCategory} onChange={(e) => setNewCategory(e.target.value)} className="w-full bg-[#1A1A1A] border border-dark-border rounded-lg px-3 py-2 text-sm text-dark-primary outline-none focus:border-cyan">
            ...
            <div>
              ...
              <input type="text" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="Note title..."
                className="w-full bg-[#1A1A1A] border border-dark-border rounded-lg px-3 py-2 text-sm text-dark-primary placeholder-[#6B6B6B] outline-none focus:border-cyan" />
            </div>
            <div>
              ...
              <textarea value={newContent} onChange={(e) => setNewContent(e.target.value)} placeholder="Write your note here..." rows={12}
                className="w-full bg-[#1A1A1A] border border-dark-border rounded-lg px-3 py-2 text-sm text-dark-primary placeholder-[#6B6B6B] outline-none focus:border-cyan resize-none" />
            </div>
